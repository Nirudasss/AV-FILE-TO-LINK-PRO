...
async def private_receive_handler(c: Client, m: Message):                    
    ...
    file_name = file_id.file_name if file_id.file_name else f"Elite_Botz_{int(time.time())}.mkv"
    file_size = get_size(file_id.file_size)
    try:
        forwarded = await m.forward(chat_id=BIN_CHANNEL)
        hash_str = get_hash(forwarded)
        stream = f"{URL}watch/{forwarded.id}/Elite_Botz_{int(time.time())}.mkv?hash={hash_str}"
